...
class Buffer:
    def __init__(self):     # Конструктор без аргументов
        ### ksenia:
        ### вместо "Buffer." лучше используйте "self.". Оно указывает на этот же класс, и если вы его, например, переименуете,
        ### не нужно будет менять его во всех методах. Ну и просто так принято.
...
#        print("Buffer.input:", a)
        ...
        ### ksenia:
        ### В питоне очень мало мест, где нужны скобки. Здесь тоже не нужны. Вы их можете ставить, но
        ### это затрудняет чтение кода для других.
        (Buffer.input).extend(a)        # Добавдяем в буфер ввода
        Length_in = len(Buffer.input)   # Длина буфера ввода
#        print("Length_in:", Length_in)

        ### ksenia:
        ## эти 2 строки ничего не делают. get_current_part возвращает input,
        ## но он ни присваивается переменной, ни выводится на экран. Т.е просто возвращается в никуда.
...
#                print("4. Buffer.input:", Buffer.input)
            ### ksenia:
            ### снова вызов get_current_part не имеет никакого смысла. Я если честно не понимаю, зачем он вообще нужен.
            Buffer.get_current_part(Buffer.input)       # Вызов метода передачи
#
